...
We will see a quick setup to configure the ``tool_santa`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_santa`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_santa`` formulas under ``/srv/salt/tool_santa/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_santa/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_santa/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_santa/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_santa/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_santa`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_santa`` formulas under ``/srv/salt/tool_santa/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_santa/parameters/roles
...
We create a configuration for the role ``tool_santa/server`` in ``/srv/salt/tool_santa/parameters/roles/tool_santa/server.yaml``:
...
We create another configuration for the role ``tool_santa/client`` in ``/srv/salt/tool_santa/parameters/roles/tool_santa/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_santa/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_santa/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_santa/parameters/dns:domain/  '/srv/salt/tool_santa/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-santa-as-json:
      test.nop:
        - name: {{ santa | json }}
...
.. code-block:: sls
    ...
    santa-config-file-file-managed:
      file.managed:
        - name: {{ santa.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='santa-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ santa.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``santa`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ santa['winner'] }}
